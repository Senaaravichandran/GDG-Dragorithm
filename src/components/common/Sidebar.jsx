  ...
  return (
    ...
    >
      <div className="text-3xl font-bold tracking-wider mt-1">
        <span className="text-[#c90000]">EvaSafe</span>
      </div>
    </motion.div>
  );
};
              ...
              <Link key={item.href} to={item.href} onClick={() => setActiveItem(item.href)}>
                <motion.div
                  className={`flex items-center p-3 text-base font-medium rounded-lg hover:bg-gray-700/40 transition-all duration-300 mb-1 relative overflow-hidden ${
                    isActive
                      ? "bg-gray-700/60 shadow-lg border-l-2 border-[#ffd700]"
                      : "bg-gray-800/20"
                  }`}
                  whileHover={{ x: 3 }}
                >
                  ...
                  >
                    <item.icon
                      size={22}
                      style={{ color: isActive ? "#ffd700" : item.color }}
                    />
                  </div>
                  ...
                  {item.name === "Alerts" && alertCount > 0 && (
                    <motion.div
                      className="ml-auto bg-amber-500 text-white text-xs font-bold px-2 py-1 rounded-full"
                      initial={{ scale: 0.8 }}
                      animate={{ scale: 1 }}
                      transition={{
                        duration: 0.3,
